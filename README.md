...
## 🚀 Como executar (A forma mais fácil)
...
graph TD
    Client([Usuário / Internet]) -->|HTTP:80| Nginx[Nginx Reverse Proxy]
    
    subgraph "Camada de Aplicação (Stateless)"
        Nginx -->|/app1 | App1[App1 - Python Gunicorn]
        Nginx -->|/app2 | App2[App2 - Node.js Express]
    end
    
    subgraph "Camada de Observabilidade"
        Prometheus[Prometheus] -->|Scrape /metrics| App1
...
Deploy em Cluster Gerenciado (Ex: AWS EKS / ECS):
...
Segurança de Acessos e Redes (VPC & IAM):
...
Gestão do Nginx Proxy e Observabilidade:
